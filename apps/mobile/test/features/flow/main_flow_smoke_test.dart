...
void main() {
  ...
  testWidgets('main flow entry point is visible for the current unlocked day',
      (tester) async {
    ...
    expect(find.textContaining('هذا اليوم ما زال مغلقاً'), findsNothing);
  });

  testWidgets('next unlocked day becomes startable after day one completion',
      (tester) async {
    final controller = AppProgressController(
      totalDays: 7,
    ...
    await tester.pumpWidget(
      _buildTestApp(
        ...
        child: HomeScreen(
          apiClient: _FlowSmokeApiClient(
            dailyPlan: _buildDailyPlan(dayNumber: 2),
            lessons: _buildLessons(dayNumber: 2),
          ),
          dayNumber: 2,
        ),
      ),
...
class _FlowSmokeApiClient extends SaxPathApiClient {
  _FlowSmokeApiClient({
